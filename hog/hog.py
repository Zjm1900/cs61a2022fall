...
def perfect_square(score):
    if int(math.sqrt(score)) ** 2 == score:
        return True
    else:
        return False
...
def play(strategy0, strategy1, update,
    ...
    while(score0 < goal and score1 < goal):
        if not who:
            zero_num_roll = strategy0(score0, score1)
            score0 = update(zero_num_roll, score0, score1, dice)
            who = 1 - who
        
        else:
            one_num_roll = strategy1(score1, score0)
            score1 = update(one_num_roll, score1, score0, dice)
            who = 1 - who
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    test_num = strategy(0, 0)
    for s1 in range(goal):
        for s2 in range(goal):
            if test_num != strategy(s1, s2):
                return False
